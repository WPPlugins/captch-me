<?php
/*
Plugin Name: Captch Me
Plugin URI: http://www.captchme.com
Description: S&eacute;curiser & mon&eacute;tiser commentaires et inscriptions avec les publicaptchas Captch Me.
Version: 1.2.2
 ...
 * --------------------------------------------------------------------------------------*/
 ...
 // Add Captch Me captcha into comment forms
 function comment_form_add_captchme() {
    ...
    if( $captchme_options['comment_enable'] == true ) {
        ...
        // Add Captch Me captcha
        echo captchme_generate_html($captchme_options['public_key'], $_GET['error_code'], $captchme_options['ssl']);
        ...
        echo "<p><div id='captchme_submit'></div></p>
            ...
            <script type='text/javascript'>
                ...
                //$('#captchme_submit').append('<br /><p>'+button.html()); //places the button after Captch Me
        ...
        if ((is_single() || is_page()) && $_GET['comment']) {
            ...
            if( $comment->comment_approved == 'spam') {
                ...
                if( $captchme_options['comment_spam_delete'] == true ) {
                    ...
                    // Remove potential captch me tag
                ...
                $text = preg_replace('/([\\/\(\)\+\;\'])/e','\'%\'.dechex(ord(\'$1\'))', $comment->comment_content);
                $text = preg_replace('/\\r\\n/m', '\\\n', $text);
 ...
 * --------------------------------------------------------------------------------------*/
 ...
 // Add Captch Me captcha into registration forms
 function registration_form_add_captchme() {
    ...
    if( $captchme_options['registration_enable'] == true ) {
        ...
        echo captchme_generate_html($captchme_options['public_key'], $captchme_error, $captchme_options['ssl']);
        // Move form submit button under the captcha
        echo "<p><div id='captchme_submit'></div></p>
 ...
 function registration_form_check_captchme($errors) {
    ...
    if( $captchme_options['registration_enable'] == true ) {
        ...
        if (empty($_POST['captchme_response_field']) || $_POST['captchme_response_field'] == '') {
            ...
            $errors->add('cm_empty_answer', '<strong>'.__('ERROR','captchme').'</strong>: '.__('Please fill an the Captch Me form.','captchme'));
            return $errors;
        }
        ...
        if (!$captchme_response->is_valid){ // && $captchme_response->error == 'invalid-response') {
            $captchme_error = $captchme_response->error;
            $errors->add('cm_wrong_answer', '<strong>'.__('ERROR','captchme').'</strong>: '.__('Captch Me captcha response was incorrect.','captchme'));
        }
...
// Check Captch Me captcha
 function login_form_check_captchme( $user, $password ) {
    ...
    if( $captchme_options['login_enable'] == true ) {
        ...
        if (empty($_POST['captchme_response_field']) || $_POST['captchme_response_field'] == '') {            
            $captchme_error = "missing-mandatory-parameter";
            return new WP_Error('cm_empty_answer', '<strong>'.__('ERROR','captchme').'</strong>: '.__('Please fill an the Captch Me form.','captchme'));
        }
        ...
        if (!$captchme_response->is_valid){ // && $captchme_response->error == 'invalid-response') {
            $captchme_error = $captchme_response->error;
            return new WP_Error('cm_wrong_answer',  '<strong>'.__('ERROR','captchme').'</strong>: '.__('Captch Me captcha response was incorrect.','captchme'));
        }
    }
    return $user;
 }
...
// Add Captch Me captcha into Contact form 7
function contact_form_7_add_captchme($tag) {
    ...
    // Add Captch Me captcha
    $output .= captchme_generate_html($captchme_options['public_key'], $captchme_error, $captchme_options['ssl']);
...
// Check captcha value
 function contact_form_7_check_captchme($comment_data, $tag) {
    ...
    if (empty($_POST['captchme_response_field']) || $_POST['captchme_response_field'] == '') {
        $captchme_error = "missing-mandatory-parameter";
        $tag->name = "captchme";
        $comment_data->invalidate($tag, __('Please fill an the Captch Me form.','captchme') . ' ' . __('If Captch Me captcha is not visible, thanks to deactivate your ad blocker and to refresh the page.','captchme'));
        return $comment_data;
    }
    ...
    if (!$captchme_response->is_valid){
        $captchme_error = $captchme_response->error;
        $tag->name = "captchme";
        $comment_data->invalidate($tag, __('Captch Me captcha response was incorrect.','captchme') . ' ' . __('If Captch Me captcha is not visible, thanks to deactivate your ad blocker and to refresh the page.','captchme'));
        return $comment_data;
    }
    return $comment_data;
 }
...
function captchme_cf7_add_tag_generator() {
    ...
    $tag_generator->add( 'captchme', 'Captch Me by Attentive Ads', 'captchme_cf7_tag_generator', array( 'nameless' => 1 ) );
}
...
function captchme_add_head() {
    ...
    if( $captchme_options['showme_enable'] == true ) {
        echo "<script type='text/javascript'>
            ...
            <script type='text/javascript'
                    src='http://api.captchme.net/api/script?key={$captchme_options['public_key']}&style=light'>
            </script>";
    }
    ...
    if( $captchme_options['leaveme_enable'] == true ) {
        echo "<script type='text/javascript'>
            ...
            <script type='text/javascript'
                    src='http://api.captchme.net/api/script?key={$captchme_options['public_key']}&style=leaver'>
            </script>";
    }
...
function captchme_add_accessme_protect($atts) {
    ...
    var CaptchmeInterstitielOptions = {
        publicKey: '{$captchme_options['public_key']}',
        urlcallback: 'none',
        lang: '{$captchme_options['lang']}',
        theme: '{$captchme_options['theme']}',
        showtitle: '{$captchme_options['title']}',
    ...
    <script type='text/javascript'
            src='http://api.captchme.net/js/captchme-interstitial-min.js'>
    </script>";
